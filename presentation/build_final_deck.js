...
/* ---------------------------------------------------------------- Slide 11: Team */
{
  ...
  const people = [
    ...
    ["Караташоглу Фырат", [
      "архитектура решения, сборка MVP, провайдер-независимый клиент LLM",
      "разбор документа: иерархия разделов, таблицы и ссылки из .docx",
      "проверка покрытия шаблона: 5 статусов раздела (заполнен / пустой / не применимо / упомянут / нет), структурные пробелы как отдельные замечания",
      "устойчивость демо (сохранение результата в Streamlit) и расширенный набор автотестов",
      "независимый прогон проекта с нуля на отдельной машине + критическое ревью качества LLM-выдачи",
    ]],
  ];
  const pw = (CW - 0.4) / 2;
  people.forEach((p, i) => {
    ...
    bullets(s, p[1], x + 0.3, 3.02, pw - 0.6, 3.05, { size: 10.5, color: P.INK2, gap: 8, lsm: 1.1 });
  });
